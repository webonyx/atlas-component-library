...
const makeItem = (index, activeIndex, setActiveIndex, size) => {
  return (
    <Item className={`paginations-item ${size}`} active={index === activeIndex} onClick={setActiveIndex(index)}>
      {index + 1}
    </Item>
  )
}
...
const makeEllipsis = (index, activeIndex, setActiveIndex, size) => {
  return <Ellipsis className={`paginations-item ${size}`} onClick={setActiveIndex(index)} />
}
...
const Paginations = ({ count, pageSize, activeIndex, onChange, config, size, ...props }) => {
  ...
  return (
    <Pagination size={size} {...props}>
      <First disabled={disabledFirst} onClick={setActiveIndex(0)} />
      <Prev disabled={disabledFirst} onClick={setActiveIndex(Math.max(0, activeIndex - 1))} />
      {range.map((item, index) => (
        <React.Fragment key={`key-${index}`}>{item}</React.Fragment>
      ))}
      <Next disabled={disabledLast} onClick={setActiveIndex(Math.min(totalPage - 1, activeIndex + 1))} />
      <Last disabled={disabledLast} onClick={setActiveIndex(totalPage - 1)} />
    </Pagination>
  )
}

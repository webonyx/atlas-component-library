...
const Popovers = ({
  ...
  const popover = (
    <Popover>
      ...
      {component && <Popover.Body>{component}</Popover.Body>}
    </Popover>
  )

  return (
    <>
      <OverlayTrigger show={backdrop ? open : undefined} overlay={popover} onHide={onHide} trigger={trigger} {...props}>
        <div className={containerClassName} onClick={() => backdrop && setOpen(!open)}>
          {children}
        </div>
      </OverlayTrigger>
      {backdrop && (
